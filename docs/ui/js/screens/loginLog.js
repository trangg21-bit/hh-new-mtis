/* ================================================================
   MTIS Login Log Screen (S-M01-06) — live API integration
   ================================================================ */
...
const SCREEN_LOGIN_LOG = {
  ...
  render() {
    return `
      <div class="content">
        ...
        <h2 class="page-title">Nhật ký đăng nhập</h2>

        <div class="card mt-4">
          <div class="search-bar">
            <input type="text" class="form-control" id="log-username" placeholder="Người dùng..."
                   value="${esc(this._username)}" style="width:160px">
  ...
  async load() {
    ...
    try {
      const data = await apiGet('/api/auth/login-log', {
        ...
        from_date: this._fromDate || undefined,
        to_date: this._toDate || undefined,
      ...
      if (this._data.length === 0) {
        ...
        tbody.innerHTML = this._data.map(log => {
          ...
          return `
            <tr>
              <td>${esc(log.logged_at)}</td>
              <td><strong>${esc(log.username)}</strong></td>
              <td>${esc(log.ip)}</td>
              <td>${esc(log.device || '—')}</td>
  ...
  _renderPagination(container, totalPages) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      html += `<button class="page-btn ${i === this._page ? 'active' : ''}" onclick="SCREEN_LOGIN_LOG.goToPage(${i})">${i}</button>`;
    }
