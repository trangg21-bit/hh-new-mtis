...
router.post('/', (req, res) => {
  ...
  const validRoles = ['system-admin', 'director', 'port-authority-leader', 'infrastructure-officer'];
  if (!u || !p || !fn) return res.status(400).json({ error: 'Thiếu trường bắt buộc' });
  if (u.length < 3) return res.status(400).json({ error: 'Tên đăng nhập tối thiểu 3 ký tự' });
  if (u.length > 20) return res.status(400).json({ error: 'Tên đăng nhập tối đa 20 ký tự' });
  ...
  if (existingUser) return res.status(409).json({ error: `Tên đăng nhập '${u}' đã tồn tại. Vui lòng chọn tên khác.` });
  // Check if email already exists
  const existingEmail = e ? db.prepare('SELECT id FROM users WHERE email = ?').get(e) : null;
  if (existingEmail) return res.status(409).json({ error: `Email '${e}' đã tồn tại. Vui lòng chọn email khác.` });
  ...
  try {
    ...
    if (e.message.includes('UNIQUE')) return res.status(409).json({ error: 'Dữ liệu đã tồn tại trong hệ thống.' });
...
router.get('/export', (req, res) => {
  ...
  res.json({
    ...
    groups,
    login_log: loginLog,
    sessions: sessions.map(s => ({ ...s, ip: s.ip ? s.ip.slice(0, 3) + '***' : null }))
  });
});
